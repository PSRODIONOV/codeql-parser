...
гарантированно видел те же файлы, что и статический анализ.
"""
...
def sensor_filter_factory(include_patterns=None, exclude_patterns=None, counters=None):
    ...
    ProjectDB.set_sensor_filters, вкладка «Динамический анализ») — для
    исключения пакетов раннего bootstrap JVM (java/lang/**,
    java/util/concurrent/**) и подобных случаев без правки самого
    инструментатора под каждый новый проект. Применяется ПОСЛЕ базового
    --pattern/include-list/exclude-list (общая область проекта, та же, что
    у статики) — сужает или
...
def extract_project_sources(db_path, dest_dir, pattern_filter=None,
    ...
    нет на диске до сборки. Заменяет необходимость передавать отдельный
    каталог-копию исходников в instrument_c_make.py/instrument_cpp.py —
    каталог для инструментации всегда есть в самой БД.
