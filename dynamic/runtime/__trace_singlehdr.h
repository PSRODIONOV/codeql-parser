...
static void __cq_write(const char *buf) {
    ...
    if (__cq_fp) {
        ...
        /* fflush убран из горячего пути: stdio-буфер (~4–8 КБ) сам
         * сбрасывается при ротации (__cq_rotate вызывает fclose/fopen),
         * при завершении процесса (atexit-чистка stdio) и при переполнении
         * буфера. Принудительный fflush на каждой записи давал сотни
         * тысяч sync-syscall при старте многопоточной VM и увеличивал
         * время холодного запуска в сотни раз. Для отладочной сессии
         * риск потери последних записей при крэше приемлем. */
    }
}
