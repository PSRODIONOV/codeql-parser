...
def is_instrumented(btype: str, outcome: str) -> bool:
    """Сработает ли датчик ветви.

    if: оба исхода датчированы — у "нет" свой номер (else_branch_num,
    физический датчик в else-блоке), не общий с "да".
    for/while/do: у исхода "нет" (цикл не выполнился ни разу) нет
    физического блока кода — там датчика нет и не может быть, остаётся
    «по да-ветвям».
    try: вход в блок происходит при любом исходе (да — try-блок, catch —
    свой catch-блок, тоже со своим номером).
    case/default: инструментируются и срабатывают при достижении (в т.ч.
    через fallthrough), поэтому считаются сработавшими."""
    if btype == "if":
        return True
    if btype in ("for", "while", "do"):
