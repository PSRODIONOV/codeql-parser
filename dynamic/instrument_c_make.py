...
def main():
    ...
    total = sid - 1
    # total здесь — ДО разрешения inline_candidate и валидации open/close
    # (которые могут ещё отбросить часть точек как "надёжного места нет",
    # см. dropped_sids ниже) — поэтому это "потенциальные" точки, а не
    # финальное число; окончательное и точное — в [OK] в конце.
    print(f"[4] Точек к размещению (потенциально): {total} в {len(touched)} файлах "
          f"(пропущено точек: {skipped})")
    ...
    for fp in touched:
        ...
        while i < len(resolved):
            ...
            if (entry[0] == "open" and i + 1 < len(resolved)
                ...
                # Последний символ тела (по координате CodeQL, c_col-1
                # 0-based) должен быть ПУНКТУАЦИЕЙ (';', ')', '}' и т.п.) —
                # этим заканчивается ЛЮБОЙ корректный одиночный оператор
                # (ExprStmt/ReturnStmt/ThrowStmt/пустой ';', GNU statement-
                # expression `({ ... })` как аргумент и т.п.). Буква/цифра/'_'
                # на этой позиции — признак, что координата обрезана ВНУТРИ
                # идентификатора: HotSpot-идиома CHECK/CHECK_/RETURN/TRAPS
                # (макрос-аргумент САМ закрывает скобки вызова — `f(...,
                # CHECK)` -> `f(..., THREAD); if (...) return; ...)`), и
                # CodeQL репортует конец оператора там, где кончается
                # макроподстановка (сразу после "CHECK"), а НЕ после
                # настоящего ');' вызова. Вставка "}" по такой координате
                # попадёт ВНУТРЬ списка аргументов вызова (см.
                # classfile_parse_error(..., CHECK) в classFileParser.cpp) —
                # пары нет надёжного места, пропускаем обе половины (как
                # макро-кейсы выше).
